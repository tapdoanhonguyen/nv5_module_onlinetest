...
function delete_type_exam(type_exam_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=type_exam&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
...
$('.formajax').on('change', function() {
	...
	var id = $(this).attr('id');
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=type_exam&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
...
function download_type_exam(obj, type_exam_id, token) {
	...
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=type_exam&action=is_download&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
		data: 'type_exam_id=' + type_exam_id + '&token=' + token,
		beforeSend: function() {
...
$('.export_file').on('click', function(e) {
	...
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=type_exam&action=is_download2&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
		data: {token: '{TOKEN}'},
		beforeSend: function() {

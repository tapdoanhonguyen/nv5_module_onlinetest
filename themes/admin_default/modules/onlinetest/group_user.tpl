					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<td class="text-center"><a {USER_MANAGER_ID_ORDER} href="{URL_USER_MANAGER_ID}"><strong>{LANG.group_user_manager}</strong></a> </td>
						...
						<tbody>
							...
							<tr id="group_{LOOP.group_user_id}">
								<td class="text-center">
									<select id="id_weight_{LOOP.group_user_id}" class="form-control btn-sm formajax" data-action="weight" data-id="{LOOP.group_user_id}" data-token="{LOOP.token}">
									<!-- BEGIN: weight -->
								...
								<td class="text-left"><a href="{LOOP.user_link}" target="_blank"><strong>{LOOP.user_manager}</strong></a></td>
								...
								<td class="text-center">
									<a href="{LOOP.user_add}" data-toggle="tooltip" data-placement="top" title="{LANG.group_user_add_user}" class="btn btn-success btn-sm"><i class="fa fa-plus"></i></a>
									<a href="{LOOP.edit}" data-toggle="tooltip" title="{LANG.edit}" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
									<a href="javascript:void(0);" onclick="delete_group_user('{LOOP.group_user_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
$('.formajax').on('change', function() {
	...
	var id = $(this).attr('id');
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=group_user&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
...
function delete_group_user(group_user_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=group_user&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',

...
$page_title = $lang_module['group_user'];
...
if( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		if( $count )
		{
			...
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_del_group_user', implode( ', ', $_del_array ), $admin_info['userid'] );
...
elseif( ACTION_METHOD == 'status' )
{
	...
	if( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $group_user_id ) )
	{
		...
		if( $db->exec( $sql ) )
		{
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_change_status_group_user', 'group_user_id:' . $group_user_id, $admin_info['userid'] );
	...
	if( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $group_user_id ) )
	{
		...
		if( $group_user_id > 0 )
		{
			...
			if( $db->exec( $sql ) )
			{
				...
				nv_insert_logs( NV_LANG_DATA, $module_name, 'log_change_weight_group_user', 'group_user_id:' . $group_user_id, $admin_info['userid'] );
...
elseif( ACTION_METHOD == 'userlist' )
{
	...
	$base_url = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=group_user&amp;sort=' . $sort . '&amp;order=' . $order . '&amp;per_page=' . $per_page;
	...
	$xtpl = new XTemplate( 'group_userlist.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file );
	...
	$xtpl->assign( 'ADD_NEW',  NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=group_user&action=add_user&token=" . md5( $nv_Request->session_id . $global_config['sitekey'] . $data['group_user_id'] ) . "&group_user_id=" . $data['group_user_id'] );
	$xtpl->assign( 'TOTAL_USER', $num_items );

	$order2 = ( $order == 'asc' ) ? 'desc' : 'asc';
	$xtpl->assign( 'URL_USERNAME', NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;sort=gu.title&amp;order=' . $order2 . '&amp;group_user_id=' . $data['group_user_id'] . '&amp;per_page=' . $per_page );
	$xtpl->assign( 'URL_FULL_NAME', NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;sort=gu.weight&amp;order=' . $order2 . '&amp;group_user_id=' . $data['group_user_id'] . '&amp;per_page=' . $per_page );
	$xtpl->assign( 'URL_ACTIVE', NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;sort=gu.status&amp;order=' . $order2 . '&amp;group_user_id=' . $data['group_user_id'] . '&amp;per_page=' . $per_page );
	...
	if( ! empty( $dataContent ) )
	{
		foreach( $dataContent as $item )
		{
			...
			$item['user_link'] = NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=users&" . NV_OP_VARIABLE . "=edit&userid=" . $item['userid'];
...
elseif( ACTION_METHOD == 'add_user' )
{
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		try
		{
			...
			if( $countInsert > 0 )
			{
				...
				nv_insert_logs( NV_LANG_DATA, $module_name, 'Add User To Group', 'group_user_id: ' . $data['group_user_id'], $admin_info['userid'] );	 
		...
		{
			$nv_Cache->delMod($module_name);
			Header( 'Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=group_user&action=userlist&group_user_id=' . $data['group_user_id'] );
			die();
		}
	...
	$xtpl = new XTemplate( 'group_user_add_user.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file );
	$xtpl->assign( 'LANG', $lang_module );
...
elseif( ACTION_METHOD == 'delete_user' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		if( $count )
		{
			...
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_del_user_from_group', implode( ', ', $_del_array ), $admin_info['userid'] );
...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	$data = array(
		...
		'user_manager_title' => '',
		'user_manager_id' => 0,
		'user_create_id' => $admin_info['userid'],
		'status' => 1,
	...
	if( $data['parent_id'] > 0 )
	{
		$data['user_manager_title'] = $db->query('SELECT username FROM ' . NV_USERS_GLOBALTABLE . ' WHERE userid = ( SELECT user_manager_id FROM '. TABLE_ONLINETEST_NAME .'_group_user WHERE group_user_id=' . intval( $data['parent_id'] ) . ')' )->fetchColumn();
	...
	if( $data['group_user_id'] > 0 )
	{
		...
		$data['user_manager_title'] = $db->query('SELECT username FROM ' . NV_USERS_GLOBALTABLE . ' WHERE userid=' . intval( $data['user_manager_id'] ) )->fetchColumn();
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		$data['user_manager_id'] = $nv_Request->get_int( 'user_manager_id', 'post', 0 );
		$data['user_manager_id'] = ( $data['user_manager_id'] ) ? $data['user_manager_id'] : $admin_info['userid'];
		...
		// if( empty( $data['user_manager_id'] ) )
		// {
			// $error['user_manager_id'] = $lang_module['group_user_error_user_manager_id'];	
		// }
			...
			try
			{
				if( $data['group_user_id'] == 0 )
				{
					...
					if( $data['group_user_id'] = $db->lastInsertId() )
					{
						...
						nv_insert_logs( NV_LANG_DATA, $module_name, 'Add A Group', 'group_user_id: ' . $data['group_user_id'], $admin_info['userid'] );	 
				...
				else
				{
					...
					if( $stmt->execute() )
					{
						...
						nv_insert_logs( NV_LANG_DATA, $module_name, 'Edit A Group', 'group_user_id: ' . $data['group_user_id'], $admin_info['userid'] );
		...
		{
			$nv_Cache->delMod($module_name);
			Header( 'Location: ' . NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=group_user&parent_id=' . $data['parent_id'] );
			die();
		}
	...
	$xtpl = new XTemplate( 'group_user_add.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file );
	$xtpl->assign( 'LANG', $lang_module );
	$xtpl->assign( 'GLANG', $lang_global );
	$xtpl->assign( 'NV_LANG_VARIABLE', NV_LANG_VARIABLE );
	$xtpl->assign( 'NV_LANG_DATA', NV_LANG_DATA );
	...
	$sql = 'SELECT group_user_id, title, lev FROM ' . TABLE_ONLINETEST_NAME . '_group_user WHERE group_user_id !=' . $data['group_user_id'] . ' ORDER BY sort ASC';

	$result = $db->query( $sql );
	
	$array_cat_list = array();
 
	while( list( $group_user_id_i, $title_i, $lev_i ) = $result->fetch( 3 ) )
	{
		$xtitle_i = '';
		if( $lev_i > 0 )
		{
		...
		$xtitle_i .= $title_i;
		$array_cat_list[] = array( $group_user_id_i, $xtitle_i );
	}
 
	foreach( $array_cat_list as $rows_i )
	{
		$xtpl->assign( 'GROUP_USER', array( 'key'=> $rows_i[0], 'name'=> $rows_i[1], 'selected'=> ( $rows_i[0] == $data['parent_id'] ) ? 'selected="selected"' : '' ) );
...
$sql = TABLE_ONLINETEST_NAME . '_group_user gu LEFT JOIN '. NV_USERS_GLOBALTABLE .' u ON (gu.user_manager_id = u.userid) WHERE gu.parent_id = ' . $parent_id;

$sort = $nv_Request->get_string( 'sort', 'get', '' );

$order = $nv_Request->get_string( 'order', 'get' ) == 'desc' ? 'desc' : 'asc';

$sort_data = array( 'gu.title', 'gu.status', 'gu.sort' );
...
$xtpl->assign( 'MODULE_NAME', $module_name );
$xtpl->assign( 'TOKEN', md5( $nv_Request->session_id . $global_config['sitekey'] ) );
$xtpl->assign( 'ADD_NEW', NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=group_user&action=add&amp;parent_id=" . $parent_id );

$order2 = ( $order == 'asc' ) ? 'desc' : 'asc';
$xtpl->assign( 'URL_TITLE', NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;sort=gu.title&amp;order=' . $order2 . '&amp;parent_id=' . $parent_id . '&amp;per_page=' . $per_page );
$xtpl->assign( 'URL_WEIGHT', NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;sort=gu.weight&amp;order=' . $order2 . '&amp;parent_id=' . $parent_id . '&amp;per_page=' . $per_page );
$xtpl->assign( 'URL_STATUS', NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $op . '&amp;sort=gu.status&amp;order=' . $order2 . '&amp;parent_id=' . $parent_id . '&amp;per_page=' . $per_page );
...
if( $parent_id > 0 )
{
	...
	while( $parentid_i > 0 )
	{
		...
		$array_cat_title[] = "<a href=\"" . NV_BASE_ADMINURL . "index.php?" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=group_user&amp;parent_id=" . $group_user_id_i . "\"><strong>" . $title_i . "</strong></a>";
...
if( ! empty( $dataContent ) )
{
	foreach( $dataContent as $item )
	{
		...
		$item['user_link'] = NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=users&" . NV_OP_VARIABLE . "=edit&userid=" . $item['group_user_id'];
		$item['user_add'] = NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=group_user&action=add_user&token=" . $item['token'] . "&group_user_id=" . $item['group_user_id'];
		$item['user_list'] = NV_BASE_ADMINURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=group_user&action=userlist&group_user_id=' . $item['group_user_id'];
		$item['link'] = NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=group_user&parent_id=" . $item['group_user_id'];
		$item['edit'] = NV_BASE_ADMINURL . "index.php?" . NV_LANG_VARIABLE . "=" . NV_LANG_DATA . "&" . NV_NAME_VARIABLE . "=" . $module_name . "&" . NV_OP_VARIABLE . "=group_user&action=edit&token=" . $item['token'] . "&group_user_id=" . $item['group_user_id'] . '&parent_id=' . $item['parent_id'];

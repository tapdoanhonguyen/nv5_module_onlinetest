...
$type_exam_id = intval( end( $array_page ) );
$number = strlen( $type_exam_id ) + 1;
$alias_url = substr( $array_op[1], 0, -$number );


if( $type_exam_id == 0 || $alias_url == '' )
{
	Header( 'Location: ' . nv_url_rewrite( NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name, true ) );
	exit();
}
...
{
	$timeout = $nv_Request->get_int( $module_data . '_test_timeout_'. $type_exam_id, 'cookie', 0 );
...
$implode = array(); 
$implode[] = 'status=1';
$implode[] = 'type_exam_id = ' . intval( $type_exam_id );
...
$query = $db->query('SELECT * FROM ' . TABLE_ONLINETEST_NAME . '_type_exam WHERE ' . $where );
...
$time_set = $nv_Request->get_int($module_data . '_' . $op . '_' . $type_exam_id, 'session');
if ( empty( $time_set ) ) 
{
	$nv_Request->set_Session( $module_data . '_' . $op . '_' . $type_exam_id, NV_CURRENTTIME);
	$db->query( 'UPDATE ' . TABLE_ONLINETEST_NAME . '_type_exam SET viewed=viewed+1 WHERE type_exam_id=' . intval( $type_exam_id ) );
}            
...
{
	$url_Permanently = nv_url_rewrite( NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=dotest/' . $dataContent['alias'] . '-' . $dataContent['type_exam_id'] . $global_config['rewrite_exturl'], true );
...
$contents = ThemeOnlineTestDoTest( $dataContent, $showTesting ); 

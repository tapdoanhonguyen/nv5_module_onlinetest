					...
					<table class="table table-bordered table-hover">
						...
						<tbody>
							...
							<tr id="group_{LOOP.userid}">
								...
								<td class="text-center">
									<a href="javascript:void(0);" onclick="delete_group_user('{LOOP.group_user_id}','{LOOP.userid}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.group_user_delete_user}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
function delete_group_user(group_user_id, userid, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=group_user&action=delete_user&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',

...
if( !empty( $dataContent ) )
{
	...
	if( $dataContent )
	{
		...
		$base_url_rewrite = nv_url_rewrite( NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=' . $dataContent['history_alias'] . '/' .  strtolower( change_alias( $dataContent['title'] ) ) . $global_config['rewrite_exturl'], true );
		...
		$contents = ThemeOnlineTestShare( $dataContent, $dataQuestion );
...
{
	$result = $db->query( '
		SELECT h.*, u.username, tx.group_exam_id, tx.group_exam_list, tx.title, tx.code, tx.config, tx.images, tx.date_added FROM ' . TABLE_ONLINETEST_NAME . '_history_essay h 
	...
	if( $dataContent )
	{
		...
		$result = $db_slave->query( 'SELECT * FROM ' . TABLE_ONLINETEST_NAME . '_history_essay_row WHERE history_essay_id=' . intval( $dataContent['history_essay_id'] ) . ' ORDER BY row_id ASC' );
		...
		$contents = ThemeOnlineTestShareEssay( $dataContent, $dataQuestion );

					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<td class="text-center"><a {TITLE_ORDER} href="{URL_TITLE}"><strong>{LANG.category_title}</strong></a> </td>
								<td class="text-center"><strong>{LANG.category_status} </strong></td>
						...
						<tbody>
							...
							<tr id="group_{LOOP.category_id}">
								<td class="text-center">
									<select id="id_weight_{LOOP.category_id}" class="form-control btn-sm formajax" data-action="weight" data-id="{LOOP.category_id}" data-token="{LOOP.token}">
									<!-- BEGIN: weight -->
								...
								<td class="text-center">
									<input name="status" id="id_status_{LOOP.category_id}" value="1" type="checkbox" data-token="{LOOP.token}" data-category_id="{LOOP.category_id}" data-action="status" {LOOP.status_checked}>
								</td>
								<td class="text-center">
...
$('.formajax').on('change', function() {
	...
	var id = $(this).attr('id');
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=category&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
		data: 'action=' + action + '&category_id=' + category_id + '&new_vid=' + new_vid + '&token='+token,
		beforeSend: function() {
		...
		success: function(json) {
			...
			if ( json['new_vid'] == 0 || json['new_vid'] == 1){
				...
				$('#id_'+action+'_'+category_id).val( json['new_vid'] );
	...
	$.ajax({
		...
		success: function(json) {
			...
			if ( json['error'] )
			{
				...
				$('#category-content').prepend('<div class="alert alert-success"><i class="fa fa-check-circle"></i> '+ json['success'] +' <i class="fa fa-times"></i></div>');
	...
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			...
			success: function(json) {
				...
				if (json['error']) {
					$('#category-content').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '<i class="fa fa-times"></i></div>');
				}

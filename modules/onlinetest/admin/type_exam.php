...
elseif( ACTION_METHOD == 'is_download' )
{
	...
	{
		$typeExam = $db->query('SELECT * FROM ' . TABLE_ONLINETEST_NAME . '_type_exam WHERE type_exam_id=' . intval( $type_exam_id ) )->fetch();
...
elseif( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		{
			$result = $db->query( 'DELETE FROM ' . TABLE_ONLINETEST_NAME . '_type_exam WHERE type_exam_id = ' . ( int )$type_exam_id );
			if( $result->rowCount() )
			{
		...
		if( $count )
		{
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_del_type_exam', implode( ', ', $_del_array ), $admin_info['userid'] );
...
elseif( ACTION_METHOD == 'getQuestion' )
{
	...
	while( $item = $result->fetch() )
	{
		...
		$json['data'][] = array( 'question_id' => $item['question_id'], 'level_id' => $item['level_id'], 'level' => $level, 'question' => html_entity_decode( $item['question'] , ENT_QUOTES, 'UTF-8' ) );
	}
	$result->closeCursor();
...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $data['type_exam_id'] > 0 )
	{
		...
		WHERE type_exam_id=' . $data['type_exam_id'] )->fetch();
		...
		elseif( $data['type_id'] == 2 && !empty( $getConfig ) ) 
		{
			...
			{
				$question_list[$question_id] = array('question_id'=> $question_id, 'answers'=> $item['answers'], 'trueanswer'=> $item['trueanswer'] );
			}		 
		}
		...
		else 
		{
			...
			{	
				...
				foreach( $getConfig as $question_id => $answers )
				{
					...
					$question_list[$question_id] = array('question_id'=> $question_id, 'answers'=> $answers, 'trueanswer'=> $trueanswer );
					++$data['num_question'];
				}
			...
			try
			{
				if( $data['type_exam_id'] == 0 )
				{
					...
					if( $data['type_exam_id'] = $db->lastInsertId() )
					{
						...
						nv_insert_logs( NV_LANG_DATA, $module_name, 'Add type exam', 'type_exam_id: ' . $data['type_exam_id'], $admin_info['userid'] );
				...
				else
				{
					...
					if( $stmt->rowCount() )
					{
						nv_insert_logs( NV_LANG_DATA, $module_name, 'Edit Type Exam' . $data['type_exam_id'], $admin_info['userid'] );
	...
	$xtpl = new XTemplate( 'type_exam_add.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file );
	...
	elseif( !empty( $getConfig ) && $data['type_id'] == 2)
	{
		foreach( $getConfig as $_question_id => $item )
		{
			...
			{			
				$xtpl->assign( 'ANS', array('key'=> $ans, 'checked' => (in_array( $ans, $trueanswer )) ? 'checked="checked"' : '', 'class_checked' => (in_array( $ans, $trueanswer )) ? 'checked' : '', 'title'=> $onlineTestTitleFirst[$ans-1]) );
...
$xtpl = new XTemplate( 'type_exam.tpl', NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $module_file );
$xtpl->assign( 'LANG', $lang_module );
$xtpl->assign( 'NV_LANG_VARIABLE', NV_LANG_VARIABLE );
$xtpl->assign( 'NV_LANG_DATA', NV_LANG_DATA );
$xtpl->assign( 'NV_BASE_SITEURL', NV_BASE_SITEURL );
...
$xtpl->assign( 'ADD_NEW', NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=type_exam&action=add' );
...
if( ! empty( $dataContent ) )
{
	...
	foreach( $dataContent as $item )
	{
		...
		$item['edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $op . '&action=edit&token=' . $item['token'] . '&type_exam_id=' . $item['type_exam_id'];

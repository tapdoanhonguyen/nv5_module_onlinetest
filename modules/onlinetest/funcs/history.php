...
if( ACTION_METHOD == 'print' )
{
	...
	$result = $db->query( '
		...
		WHERE h.userid = '. intval( $globalUserid ) .' AND h.history_id=' . intval( $history_id ) );
...
elseif( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		{
			$result = $db->query( 'UPDATE ' . TABLE_ONLINETEST_NAME . '_history SET is_deleted=1 WHERE history_id = ' . ( int )$history_id );
			if( $result->rowCount() )
			{
		...
		if( $count )
		{
			...
			if( $redirect )
			{
				...
				$json['link'] = NV_BASE_SITEURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=history';
...
elseif( ACTION_METHOD == 'continue' )
{
	...
	{
		$history = $db->query( 'SELECT h.*, tx.title FROM ' . TABLE_ONLINETEST_NAME . '_history h LEFT JOIN ' . TABLE_ONLINETEST_NAME . '_type_exam tx ON (h.type_exam_id = tx.type_exam_id) WHERE h.history_id=' . intval( $history_id ) )->fetch();
		if ( $history && ( $history['test_time'] + ( $history['time'] * 60 ) ) > NV_CURRENTTIME )
		{
			$nv_Request->set_Session( 'history_' . $history['type_exam_id'], intval( $history_id ) );
			
			$json['link'] = nv_url_rewrite( NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=dotest/' . strtolower( change_alias( $history['title'] ) ) . '-' . $history['type_exam_id'] . $global_config['rewrite_exturl'], true );
		...
		else
		{
			if( ( $history['test_time'] + ( $history['time'] * 60 ) ) <= NV_CURRENTTIME && $history['is_sended'] == 0 ) 
			{
				...
				$db->exec( 'UPDATE ' . TABLE_ONLINETEST_NAME . '_history SET is_sended=1, time_do_test=' . intval( $history['time_do_test'] ) . ' WHERE history_id=' . intval( $history_id ) );
		 
				 
			}
			
			$nv_Request->set_Session( 'history_' . $history['type_exam_id'], 0 );	
...
$base_url = NV_BASE_SITEURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&amp;' . NV_OP_VARIABLE . '=history&amp;sort=' . $data['sort'] . '&amp;order=' . $data['order'] . '&amp;per_page=' . $onlineTestConfig['perpage'];
...
$contents = ThemeOnlineTestHistory( $dataContent, $data, $generate_page ); 
